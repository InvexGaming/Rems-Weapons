...
{
	name = "CS:GO Weapon Paints Reloaded",
...
public OnPluginStart()
{
	...
	CreateConVar("sm_wpaints_version", VERSION, "", FCVAR_PLUGIN|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_CHEAT|FCVAR_DONTRECORD);
...
public Action:Command_QuickSelect_WSkin(client, args) 
{
  ...
  if (!(isVIP)) {
    if (args != 1 || quickNumber != 0) {
      CPrintToChat(client, " {green}[WP]{default} %t", "Must be VIP");
      return Plugin_Handled;
    }
  }
  
  //Show menu if no args, otherwise perform quick select
  new Float:inputWear = -1.0;
  ...
  if (args == 0) {
    ...
    if (inputWear < 0.0 || inputWear > 1.0) {
      CPrintToChat(client, " {green}[WP]{default} %t", "Wear Value Wrong");
      return Plugin_Handled;
    }
  }
  else if (args > 2){
...
public DIDMenuHandler(Handle:menu, MenuAction:action, client, itemNum) 
{
	...
	{
    WSkin_Selecter(TYPE_MENU, menu, client, itemNum, 0, -1.0);
	}
}
...
WSkin_Selecter(type, Handle:menu, client, itemNum, quickNumber, Float:inputWear) 
{
  ...
  if (type == TYPE_MENU) {
    ...
    {
      CPrintToChat(client, " {green}[WP]{default} %t", "Index out of Range", g_paintCount - 1);
      return;
    }
  }
...
public Action:Pasado(Handle:timer, Handle:pack)
{
	...
	if(GetPlayerWeaponSlot(client, CS_SLOT_PRIMARY) == weapon || GetPlayerWeaponSlot(client, CS_SLOT_SECONDARY) == weapon || GetPlayerWeaponSlot(client, CS_SLOT_KNIFE) == weapon || (g_c4 && GetPlayerWeaponSlot(client, CS_SLOT_C4) == weapon))
	{
      ...
      case 512: strcopy(Classname, 64, "weapon_knife_falchion");
		...
		ChangePaint(client, weapon, Classname, weaponindex, -1.0);
	}
}
...
CrearArbol(client, String:cookie1[100], String:cookie2[100])
{
  ...
  SetTrieValue(arbol[client], "weapon_knife_falchion", StringToInt(parte2[15]));
	SetTrieValue(arbol[client], "weapon_knife_butterfly", StringToInt(parte2[16]));
	SetTrieValue(arbol[client], "weapon_c4", StringToInt(parte2[17]));
